...
def stft(data, fft_size=512, step_size=160, padding=True):
    ...
    window = np.concatenate((np.zeros(
        (56, )), np.hanning(fft_size - 112), np.zeros((56, ))),
                            axis=0)
...
def generate_one_sample(audio_path_list, num_speaker, fix_sr=16000, verbose=0):
    ...
    for i in range(num_speaker):
        cRM = np.divide(F_list[i],
                        F_mix,
                        out=np.zeros_like(F_list[i]),
                        where=F_mix != 0)
        y_sample[:, :, :, i] = cRM
...
def phase_enhance_pred(mix_STFT, pred_file, mode='STFT'):
    ...
    if mode == 'STFT':
        F_pred = pred_file
    M = np.sqrt(np.square(F_pred[:, :, 0]) +
                np.square(F_pred[:, :, 1]))  #magnitude
...
if __name__ == '__main__':
    ...
    if CHECK1:
        ...
        X_data, y_data = generate_dataset(sample_range,
                                          repo_path,
                                          num_speaker=2,
                                          verbose=1)
    ...
    if CHECK2:
        ...
        print(
            SNR(original_audio_path + '/trim_audio_train24.wav',
                original_audio_path + '/trim_audio_train24.wav'))
        #print(SNR(original_audio_path+'/trim_audio_train340.wav',predict_audio_path1+'/00009-00340-00340.wav'))
        print(
            SNR(original_audio_path + '/trim_audio_train340.wav',
                predict_audio_path2 + '/00009-00340-00340.wav'))

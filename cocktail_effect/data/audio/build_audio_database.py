...
@timit
def generate_path_list(sample_range=SAMPLE_RANGE,
                       repo_path=WAV_REPO_PATH,
                       frame_path=FRAME_LOG_PATH):
    '''
    :param sample_range:
    :param repo_path:
...
def split_to_mix(audio_path_list,
                 database_repo=DATABASE_REPO_PATH,
                 partition=2):
    ...
    while ((head + part_len) < length):
        ...
        with open('%s/single_TF_part%d.txt' % (database_repo, part_idx),
                  'a') as f:
            for idx, _ in part:
                name = 'single-%05d' % idx
                f.write('%s.npy' % name)
...
def train_test_split(dataset_log_path,
                     data_range=[0, 50000],
                     test_ratio=0.1,
                     shuffle=True,
                     database_repo=DATABASE_REPO_PATH):
